...
jQuery(document).ready(function($){
	...
	$(document.body).on('click.tsOpenMediaManager', '.ts-open-media', function(e){
		 ...
		 */
		ts_media_frame = wp.media.frames.ts_media_frame = wp.media({
			/**
			 ...
			 */
			className: 'media-frame ts-media-frame',
			 ...
			 * the script with the object 'ts_nmp_media' that holds our
			 * localized stuff and such. Let's populate the title with our custom
			 * text.
			 */
			title: ts_nmp_media.title,
		 ...
		 */
		ts_media_frame.on('select', function(){
			// Grab our attachment selection and construct a JSON representation of the model.
			var media_attachment = ts_media_frame.state().get('selection').first().toJSON();

			// Send the attachment URL to our custom input field via jQuery.
			//$('#ts-new-media-image').val(media_attachment.url);  // when only one media input exists
			...
			if ( inputfield.length > 0 ) inputfield.val(media_attachment.url);
